...
import './FilterJobList.css'
import { useDispatch, useSelector } from "react-redux";
import { useEffect } from "react";
import { getAllJobPosts, selectAllJobPosts } from "./jobSlice";
import { useState } from "react";

const FilterJobList = ({  jobList }) => {


    const [searchLocation, setSearchLocation] = useState("");

    const filteredJobs = jobList.filter((jobPost) => {
      const lowercaseLocation =
        typeof jobPost.location === "string"
          ? jobPost.location.toLowerCase()
          : "";
  
      return lowercaseLocation.includes(searchLocation.toLowerCase());
    });


  return (
    <>
    <div class="container py-1 jobList mb-5 ">
        <div class="container">
